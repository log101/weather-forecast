...
export default {
    ...
    async mounted() {
        ...
        if(this.$route.query.city == undefined || this.$route.query.city == null) {
            ...
            await axios.get('https://api.openweathermap.org/data/2.5/weather?q=' + city + '&appid=' + process.env.VUE_APP_API_KEY +'&lang=tr&units=metric')
                .then(response => {
                    // console.log("anlik:"+response)
            ...
            await axios.get('https://api.openweathermap.org/data/2.5/forecast?q=' + city + '&appid=' + process.env.VUE_APP_API_KEY +'&units=metric&lang=tr')
                .then(response => {
                    // console.log("gunluk:"+response)
            ...
            await axios.get('https://api.openweathermap.org/data/3.0/onecall?lat=38.963745&lon=35.243322&exclude=current,minutely,daily,alerts&appid=' + process.env.VUE_APP_API_KEY +'&units=metric')
                .then(response => {
                    // console.log("saatlik:"+response)
